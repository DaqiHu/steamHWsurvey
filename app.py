...
def classify_nv_gpu(gpu_name):
    ...
    return is_20_plus, is_high_perf
...
if not current_month_data.empty:
    ...
    col3.metric(f"问题三：主流显存容量 (全平台)", f"{top_vram_name} ({top_vram_val:.2%})")
...
st.markdown("观察特性的普及速度。如果趋势线呈现陡峭上升，说明该硬件特性正在形成绝对主流，我们的技术管线可以随之激进；如果是平缓上升或停滞，则说明下沉市场仍不可忽视。")
...
def render_trend_charts(trend_df):
    ...
    with col_a:
        ...
        st.line_chart(trend_df[["Turing 及以上架构占比 (N卡)", "≥ 2080/3060 性能占比 (N卡)"]])
